...
- Request statistics: `GET /stats` (JSON) and `GET /metrics` (Prometheus) report
  per-upstream request counts and percentages, fallback reasons, and latency
  (sum/avg/min/max, p50/p90/p99, and a Prometheus histogram). Configured via
  `[stats]` (`disabled`, `buckets`); both endpoints honour `client_token`.
...
[0.2.0]: https://github.com/al-maisan/aiproxy/compare/v0.1.0...v0.2.0
